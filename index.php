<!DOCTYPE html>

<link rel="stylesheet" type="text/css" href="index.css" media="screen" />
<link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">

<head>
    <meta charset="utf-8" />
    <title>Baptiste Thomann's DevBlog</title>
</head>

<body class = "bgacc">
    

    <div class="w3-bar w3-black w3-hide-small">
        <a href="connexion.php" class="w3-bar-item w3-button w3-right">Connexion</i></a>
        <a href="contact.php" class="w3-bar-item w3-button w3-right">Contact</i></a>
        <a href="articles.php" class="w3-bar-item w3-button w3-right">Articles</i></a>
        <a href="index.php" class="w3-bar-item w3-button w3-right">Accueil</i></a>
    </div>

    <section>
        <div class="container">
            <p class="desc1">Devblog<br/>Ete 2020.</p>
            <a class="txt">Baptiste Thomann.</a>
        </div>
        <p class="desc2">Ce blog est destiné pour tous les fan de Harry Potter et de l'univer de J.K Rowling</p>

    </section>

    <footer>
        <div class="w3-bar w3-black w3-bottom w3-hide-small">
        <a href="contact.php" class="w3-bar-item w3-button w3-right"><i>baptiste.thomann@iseg.fr</i></a>
    </div>
    </footer>
</body>
</html>